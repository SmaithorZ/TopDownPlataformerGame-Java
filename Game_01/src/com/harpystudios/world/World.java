...
import com.harpystudios.entities.Bullet;
import com.harpystudios.entities.Enemy;
import com.harpystudios.entities.Entity;
import com.harpystudios.entities.Lifepack;
import com.harpystudios.entities.Player;
import com.harpystudios.entities.Weapon;
...
public class World {
			...
			map.getRGB(0, 0, map.getWidth(), map.getHeight(), pixels, 0, map.getWidth());
			{
			   for(int xx = 0; xx < map.getWidth(); xx++) {
				for(int yy = 0; yy < map.getHeight(); yy++) {
					...
					}else if(pixelAtual == 0xFFb300ff) {
						//Enemy
					Enemy en = new Enemy(xx*16,yy*16,16,16,Entity.ENEMY_EN);	
					Game.entities.add(en);
					Game.enemies.add(en);
						
					}else if(pixelAtual == 0xFFFF21de) {
						//Lifepack
						Lifepack pack = new Lifepack(xx*16,yy*16,16,16,Entity.LIFEPACK_EN);
						Game.entities.add(pack);
						pack.setMask(8,8,8,8);
					}else if(pixelAtual ==0xFF8f4560) {
						//Weapon
						Game.entities.add(new Weapon(xx*16,yy*16,48,16,Entity.WEAPON_EN));
					}else if(pixelAtual == 0xff12ffff) {
						//Bullet
						Game.entities.add(new Bullet(xx*16,yy*16,64,16,Entity.BULLET_EN));
	...
	public static void restartGame(String level) {
		Game.entities = new ArrayList<Entity>();
		  ...
		  Game.spritesheet = new Spritesheet("/Sprite_Player.png");
